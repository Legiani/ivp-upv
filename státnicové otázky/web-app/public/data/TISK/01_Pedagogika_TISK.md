...
```mermaid
graph TB
    subgraph SOC["🌍 SOCIALIZACE (celoživotní proces)"]
        direction TB
        subgraph VYCH["🎯 VÝCHOVA (záměrná, řízená)"]
            direction LR
            V_S["Výchova v širším smyslu"]
            V_U["Výchova v užším smyslu<br/>(formování charakteru,<br/>postojů, hodnot)"]
            VZD["VZDĚLÁVÁNÍ<br/>(proces získávání<br/>vědomostí a dovedností)"]
            V_S --> V_U
            V_S --> VZD
        end
        INF["Informální učení<br/>(média, vrstevníci,<br/>každodenní zkušenost)"]
        SEBV["SEBEVÝCHOVA<br/>(autoregulace)"]
        VYCH -->|"vede k"| SEBV
    end

    VZD -->|"výsledek"| VZDEL["📜 VZDĚLÁNÍ<br/>(kvalifikace, diplom)"]

    style SOC fill:#1a1a2e,stroke:#e94560,color:#fff
    style VYCH fill:#16213e,stroke:#0f3460,color:#fff
    style SEBV fill:#0f3460,stroke:#e94560,color:#fff
    style VZDEL fill:#533483,stroke:#e94560,color:#fff
```
...
```mermaid
graph LR
    I0["ISCED 0<br/>MŠ"] --> I1["ISCED 1<br/>1.–5. tř. ZŠ"]
    I1 --> I2["ISCED 2<br/>6.–9. tř. ZŠ"]
    I2 --> I3["ISCED 3<br/>SŠ / SOU / Gym"]
    I3 --> I4["ISCED 4<br/>Nástavba"]
    I3 --> I5["ISCED 5<br/>VOŠ"]
    I3 --> I6["ISCED 6<br/>Bc."]
    I6 --> I7["ISCED 7<br/>Mgr./Ing."]
    I7 --> I8["ISCED 8<br/>Ph.D."]
    I4 -.->|"možnost"| I6

    style I0 fill:#e8d5b7,stroke:#b8860b,color:#333
    style I1 fill:#f0e68c,stroke:#b8860b,color:#333
    style I2 fill:#ffd700,stroke:#b8860b,color:#333
    style I3 fill:#ffa500,stroke:#b8860b,color:#fff
    style I4 fill:#ff8c00,stroke:#b8860b,color:#fff
    style I5 fill:#ff6347,stroke:#b8860b,color:#fff
    style I6 fill:#dc143c,stroke:#b8860b,color:#fff
    style I7 fill:#8b0000,stroke:#b8860b,color:#fff
    style I8 fill:#4b0082,stroke:#b8860b,color:#fff
```
...
```mermaid
graph LR
    subgraph AUT["❌ AUTORITATIVNÍ"]
        A1["Učitel rozkazuje"]
        A2["Žák poslouchá"]
        A3["Výkon jen pod kontrolou"]
        A1 --> A2 --> A3
    end

    subgraph LIB["⚠️ LIBERÁLNÍ"]
        L1["Učitel nezasahuje"]
        L2["Žák je nejistý"]
        L3["Chaos, nízký výkon"]
        L1 --> L2 --> L3
    end

    subgraph DEM["✅ DEMOKRATICKÝ"]
        D1["Učitel vede a naslouchá"]
        D2["Žák je aktivní partner"]
        D3["Optimální rozvoj osobnosti"]
        D1 --> D2 --> D3
    end

    style AUT fill:#8b0000,stroke:#ff4444,color:#fff
    style LIB fill:#8b8000,stroke:#ffff44,color:#fff
    style DEM fill:#006400,stroke:#44ff44,color:#fff
```
...
```mermaid
graph TB
    subgraph KOM["KOMENSKÝ (17. století)"]
        K1["Škola mateřská<br/>0–6 let"] --> K2["Škola obecná<br/>6–12 let"]
        K2 --> K3["Gymnázium<br/>12–18 let"]
        K3 --> K4["Akademie<br/>18–24 let"]
    end

    subgraph SOC["SOUČASNOST (ČR)"]
        S1["MŠ<br/>ISCED 0"] --> S2["ZŠ 1. stupeň<br/>ISCED 1"]
        S2 --> S3["ZŠ 2. stupeň<br/>ISCED 2"]
        S3 --> S4["SŠ / Gymnázium<br/>ISCED 3"]
        S4 --> S5["VŠ Bc./Mgr.<br/>ISCED 6–7"]
    end
    ...
    style KOM fill:#2c1810,stroke:#d4a574,color:#fff
    style SOC fill:#1a1a2e,stroke:#e94560,color:#fff
```
...
```mermaid
mindmap
  root((DIDAKTICKÉ ZÁSADY<br/>dle Komenského))
    Názornost
      Zapojit smysly
      Orbis Pictus
      Dnes: multimédia, VR
    Systematičnost
      ...
      Postupnost a návaznost
    Přiměřenost
      Věku žáka
      Schopnostem
      Individualizace
    Aktivnost
      Vlastní činnost žáka
      Škola hrou
      Dnes: projektová výuka
    Trvalost
      Opakování
      Propojení teorie a praxe
      Pochopení > memorování
```
...
```mermaid
graph TB
    SKOLA["🏫 ŠKOLA"] --> F1["📚 Kvalifikační<br/>vědomosti, dovednosti"]
    SKOLA --> F2["🤝 Socializační<br/>spolupráce, normy"]
    SKOLA --> F3["🌍 Integrační<br/>rovný přístup"]
    SKOLA --> F4["📊 Selektivní<br/>hodnocení, třídění"]
    SKOLA --> F5["🛡️ Ochranná<br/>bezpečné prostředí"]
    SKOLA --> F6["🌱 Personalizační<br/>rozvoj individuality"]

    style SKOLA fill:#1a1a2e,stroke:#e94560,color:#fff
    ...
    style F6 fill:#16213e,stroke:#0f3460,color:#fff
```
...
```mermaid
graph TB
    subgraph SPP["🏫 ŠKOLNÍ PORADENSKÉ PRACOVIŠTĚ"]
        VP["Výchovný poradce<br/>• SVP, kariérové poradenství<br/>• povinně na každé škole"]
        SMP["Školní metodik prevence<br/>• rizikové chování<br/>• preventivní program"]
        SP["Školní psycholog<br/>• diagnostika<br/>• intervence"]
        SSP["Šk. speciální pedagog<br/>• žáci se SVP<br/>• reedukace"]
    end

    subgraph EXT["🔗 EXTERNÍ SPOLUPRÁCE"]
        PPP["PPP<br/>Ped.-psych. poradna"]
        SPC["SPC<br/>Spec.-ped. centrum"]
        OSPOD["OSPOD"]
        SVP2["SVP<br/>Stř. výchovné péče"]
    end
    ...
    style SPP fill:#0f3460,stroke:#e94560,color:#fff
...
```mermaid
mindmap
  root((ENVIRONMENTÁLNÍ<br/>VÝCHOVA))
    Průřezové téma RVP
      Integrováno do všech předmětů
      Odborné předměty = klíčové
      Příroda + Technika + Ekonomika
    Cíle
      Znalosti ekol. zákonitostí
      Odpovědné postoje
      Dovednosti pro udržitelnost
      Hodnoty TUR
    Metody
...
```mermaid
graph TB
    CILE["🎯 CÍLE<br/>Proč se učíme?"] --> OBSAH
    UCITEL["👩‍🏫 UČITEL<br/>Řídí proces"] --> OBSAH["📖 OBSAH<br/>Co se učíme?"]
    UCITEL --> ZAK["🧑‍🎓 ŽÁK<br/>Aktivní subjekt"]
    OBSAH --> ZAK
    UCITEL --> METODY["⚙️ METODY<br/>Jak se učíme?"]
    METODY --> ZAK
    PROSTREDI["🏫 PROSTŘEDÍ<br/>Kde se učíme?"] --> ZAK
    PROSTREDI --> UCITEL

    style CILE fill:#e94560,stroke:#fff,color:#fff
    style UCITEL fill:#0f3460,stroke:#fff,color:#fff
    style ZAK fill:#16213e,stroke:#fff,color:#fff
    style OBSAH fill:#533483,stroke:#fff,color:#fff
    style METODY fill:#1a1a2e,stroke:#fff,color:#fff
    style PROSTREDI fill:#2c3e50,stroke:#fff,color:#fff
```
...
```mermaid
graph LR
    SA["1️⃣ Situační<br/>analýza"] --> CILE["2️⃣ Stanovení<br/>cílů"]
    CILE --> UCIVO["3️⃣ Výběr<br/>učiva"]
    UCIVO --> METODY["4️⃣ Volba<br/>metod"]
    METODY --> PROSTREDKY["5️⃣ Volba<br/>prostředků"]
    PROSTREDKY --> FIXACE["6️⃣ Plán<br/>fixace"]
    FIXACE --> HODNOCENI["7️⃣ Způsob<br/>hodnocení"]
    HODNOCENI --> CAS["8️⃣ Časový<br/>plán"]
    CAS --> PRIPRAVA["9️⃣ Písemná<br/>příprava"]

    style SA fill:#1a1a2e,stroke:#e94560,color:#fff
    style CILE fill:#16213e,stroke:#e94560,color:#fff
    style UCIVO fill:#0f3460,stroke:#e94560,color:#fff
    style METODY fill:#533483,stroke:#e94560,color:#fff
    style PROSTREDKY fill:#5b2c6f,stroke:#e94560,color:#fff
    style FIXACE fill:#6c3483,stroke:#e94560,color:#fff
    style HODNOCENI fill:#7d3c98,stroke:#e94560,color:#fff
    style CAS fill:#8e44ad,stroke:#e94560,color:#fff
    style PRIPRAVA fill:#9b59b6,stroke:#e94560,color:#fff
```
...
```mermaid
gantt
    title Struktura vyučovací hodiny (45 min)
    dateFormat YYYY-MM-DD HH:mm
    axisFormat %H:%M

    section Faze hodiny
    Organizacni uvod       :org, 2024-01-01 08:00, 3m
    Motivace a opakovani   :mot, after org, 7m
    Expozice noveho uciva  :exp, after mot, 18m
    Fixace a procvicovani  :fix, after exp, 12m
    Diagnostika            :dia, after fix, 3m
    Zaver a shrnuti        :zav, after dia, 2m
```
...
```mermaid
timeline
    title Klíčové milníky v dějinách pedagogiky
    section Antika
        5. stol. př.n.l. : Sokrates — maieutika
        4. stol. př.n.l. : Platón — Akadémie
                         : Aristoteles — empirie
    section Středověk
        12. stol. : Vznik univerzit
                  : Scholastika
    section Novověk
        17. stol. : Komenský — Didactica Magna
        18. stol. : Locke — tabula rasa
                  : Rousseau — Émile
                  : Pestalozzi — hlava, srdce, ruka
    section 19. století
        1806 : Herbart — pedagogika jako věda
        1860s : Herbartismus — tradiční škola
    section Reformy 20. stol.
        1896 : Dewey — pragmatismus
        1907 : Montessori — Casa dei Bambini
        1919 : Steiner — Waldorf
        1920s : Parkhurstová — Dalton
    section Současnost
        2004 : ČR — Školský zákon, RVP
        2020s : Digitalizace, AI ve vzdělávání
```
...
```mermaid
graph TB
    VP["🔬 VĚDECKÉ POZNÁNÍ<br/>(akademické disciplíny)"] -->|"výběr, zjednodušení"| RVP["📋 RVP<br/>(Rámcový vzdělávací program)<br/>aktér: stát / NPI"]
    RVP -->|"konkretizace"| SVP["📝 ŠVP<br/>(Školní vzdělávací program)<br/>aktér: škola"]
    SVP -->|"didaktická analýza"| HODINA["👩‍🏫 UČIVO V HODINĚ<br/>(realizované kurikulum)<br/>aktér: učitel"]
    HODINA -->|"proces učení"| ZAK["🧑‍🎓 OSVOJENÉ POZNATKY<br/>(dosažené kurikulum)<br/>aktér: žák"]

    SKRYTE["👻 SKRYTÉ KURIKULUM<br/>(implicitní normy, kultura školy)"] -.->|"ovlivňuje"| ZAK

    style VP fill:#1a1a2e,stroke:#e94560,color:#fff
    style RVP fill:#16213e,stroke:#0f3460,color:#fff
    style SVP fill:#0f3460,stroke:#e94560,color:#fff
    style HODINA fill:#533483,stroke:#e94560,color:#fff
    style ZAK fill:#e94560,stroke:#fff,color:#fff
    style SKRYTE fill:#333,stroke:#999,color:#ccc
```
...
```mermaid
graph LR
    subgraph TRANSM["📦 Transmisivní model"]
        direction LR
        U["👩‍🏫 Učitel<br>(expert)"] -->|"předává<br>hotové vědomosti"| Z["🧑‍🎓 Žák<br>(pasivní příjemce)"]
    end

    subgraph KONSTR["🏗️ Konstruktivistický model"]
        direction LR
        U2["👩‍🏫 Učitel<br>(průvodce)"] -.->|"vytváří<br>prostředí"| PRE["🧠 Prekoncepty"]
        PRE -->|"interakce"| Z2["🧑‍🎓 Žák<br>(aktivní tvůrce)"]
        Z2 -->|"konstruuje"| POZ["💡 Nové poznání"]
    end

    style TRANSM fill:#8b0000,stroke:#e94560,color:#fff
    style KONSTR fill:#006400,stroke:#44ff44,color:#fff
```
...
```mermaid
graph TD
    A["🟢 Aktuální úroveň vývoje<br>(Co žák zvládne úplně sám)"]
    B["🟡 Zóna nejbližšího vývoje (ZPD)<br>(Co žák zvládne s pomocí učitele nebo vrstevníků)"]
    C["🔴 Frustrační zóna<br>(Co je pro žáka zatím příliš těžké i s pomocí)"]
...
```mermaid
graph TB
    P["📅 1. PŘÍPRAVA (Před učením)
    - Analýza úkolu
    - Stanovení cílů
    - Plánování času a strategie"]
    
    V["⚙️ 2. VÝKON (Během učení)
    - Aplikace učebních strategií
    - Monitorování pozornosti
    - Úprava postupu"]
    
    R["🔍 3. REFLEXE (Po učení)
    - Sebehodnocení výkonu
    - Hledání příčin úspěchu/neúspěchu
    - Poučení pro příště"]

    P -->|"začíná se učit"| V
    V -->|"dokončí práci"| R
    R -->|"zkušenost pro<br>další úkol"| P
...
```mermaid
graph BT
    L1["1️⃣ ZAPAMATOVAT<br>Vyjmenovat definice, fakta"]
    L2["2️⃣ POROZUMĚT<br>Vysvětlit svými slovy"]
    L3["3️⃣ APLIKOVAT<br>Použít znalost v praxi/nové situaci"]
    L4["4️⃣ ANALYZOVAT<br>Rozložit na části, hledat vztahy a příčiny"]
    L5["5️⃣ HODNOTIT<br>Posoudit správnost, obhájit názor"]
    L6["6️⃣ TVOŘIT<br>Složit prvky do nového originálního celku"]
    ...
    style L6 fill:#ff6b6b,stroke:#fff,color:#fff
```
...
```mermaid
graph LR
    C["🎯 VÝUKOVÝ CÍL<br>Co se žák naučí?"]
    M["⚙️ METODA A FORMA<br>Jak to budeme trénovat?"]
    H["✅ HODNOCENÍ<br>Jak to ověříme?"]

    C -->|"Určuje výběr"| M
    C -->|"Definuje kritéria"| H
    H -.->|"Poskytuje zpětnou vazbu k cílům"| C
