...
graph TD
    ...
    PSY --> A["🔄 Psychické PROCESY<br>(Dynamické, dějí se teď)"]
    PSY --> B["⏳ Psychické STAVY<br>(Dočasné naladění)"]
    PSY --> C["🗿 Psychické VLASTNOSTI<br>(Trvalé znaky osobnosti)"]
    
    A --> A1["Poznávací (Vnímání, Myšlení)"]
    A --> A2["Paměťové"]
    A --> A3["Motivační a Citové"]
    
    B --> B1["Pozornost"]
    B --> B2["Únava, Nálada, Strach"]
    
    C --> C1["Temperament"]
    C --> C2["Charakter"]
    
    style PSY fill:#533483,color:#fff
...
```mermaid
graph LR
    P["POJEM<br>(Např. 'Stroj')"] --> S["SOUD<br>(Spojení pojmů:<br>'Stroj usnadňuje práci')"]
    S --> U["ÚSUDEK<br>(Vyvození závěru:<br>'Tedy bagr je stroj')"]
    
    style P fill:#1a1a2e,stroke:#fff,color:#fff
    style S fill:#006400,stroke:#fff,color:#fff
    style U fill:#8b0000,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    subgraph KOD ["1. Kódování (Vstup)"]
        A["Smysly (Zrak, Sluch)"] -->|Pozornost| B["Krátkodobá paměť<br>(Max 30 sekund)"]
    end
    
    subgraph RET ["2. Uchování (Retence)"]
        B -->|Opakování + Emoce| C["Dlouhodobá paměť<br>(Neomezená kapacita)"]
    end
    
    subgraph REP ["3. Vybavování (Výstup)"]
        C -->|Zkoušení / Test| D["Reprodukce informací"]
    end
    
    style KOD fill:#1a1a2e,stroke:#fff,color:#fff
    style RET fill:#006400,stroke:#fff,color:#fff
    style REP fill:#8b0000,stroke:#fff,color:#fff
```
...
```mermaid
graph LR
    P1["👀 Neúmyslná poz.<br>(Silný stimul)"] -->|"Učitel zadá jasný úkol"| P2["🧠 Úmyslná poz.<br>(Zapojení vůle)"]
    P2 -->|"Činnost žáka pohltí"| P3["🌊 Poúmyslná poz.<br>(Stav 'Flow')"]
    
    style P1 fill:#e94560,stroke:#fff,color:#fff
    style P2 fill:#0f3460,stroke:#fff,color:#fff
    style P3 fill:#27ae60,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    A["1. Vnější autorita<br>(Učitel říká: 'Nesmíš opisovat!')"] --> B["2. Vnější motivace<br>(Neopisuji, protože se bojím pětky.)"]
    B --> C["3. Identifikace se vzorem<br>(Můj oblíbený mistr nešidí práci, já taky nebudu.)"]
    C --> D["4. Interiorizace - Vnitřní svědomí<br>(Neopisuji a nešidím, protože mi to nedovolí má vlastní hrdost.)"]
    
    style A fill:#8b0000,stroke:#fff,color:#fff
    style D fill:#006400,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    A["⛔️ Amotivace<br>(Nedělám nic, nechci)"] --> B["🥕 Vnější regulace<br>(Učím se, aby mě otec nezbil)"]
    B --> C["🏆 Introjekce<br>(Učím se, protože by mi jinak bylo hanba před třídou)"]
    C --> D["🧠 Identifikace<br>(Učím se, protože chápu, že mi to k něčemu bude)"]
    D --> E["🔥 VNITŘNÍ MOTIVACE<br>(Učím se, protože mě to obrovsky baví)"]
...
```mermaid
graph LR
    I1(("Interakce 1<br>(Úsměv)"))
    I2(("Interakce 2<br>(Pomoc s úkolem)"))
    I3(("Interakce 3<br>(Pochvala)"))
    
    I1 --> V["Dlouhodobý<br>SOCIÁLNÍ VZTAH<br>(Důvěra)"]
    I2 --> V
    I3 --> V
    
    V -.->|"Určuje podobu budoucích interakcí"| I4(("Interakce 4"))
    
    style V fill:#533483,stroke:#fff,color:#fff
```
...
graph TD
    ...
    H["🌟 HVĚZDA TŘÍDY<br>(Karel)"]
    A["Anna"]
    B["Pavel"]
    C["Jana"]
    D["Odmítnutý (Outsider)<br>(Michal)"]
...
```mermaid
graph TD
    A["NÁROČNÉ ŽIVOTNÍ SITUACE"]
    
    A --> B["💥 STRES<br>(Zátěž přesahuje aktuální<br>kapacity organismu)"]
    A --> C["🚧 FRUSTRACE<br>(Překážka v dosažení cíle)"]
    A --> D["🚫 DEPRIVACE<br>(Dlouhodobé nenaplnění potřeby)"]
    
    B --> B1["Eustres (Pozitivní, sport)"]
    B --> B2["Distres (Negativní, šikana)"]
...
```mermaid
graph LR
    O(("Osobnost Žáka<br>(Jirka, 17 let)"))
    
    R1["Role Dítěte<br>(Očekávání rodičů: 'Buď doma do 20:00!')"]
    R2["Role Agresora<br>(Očekávání party: 'Pojď s námi rozbít zastávku!')"]
    R3["Role Studenta<br>(Očekávání školy: 'Uč se na zkoušku!')"]
    
    R1 --> O
    R2 --> O
    R3 --> O
    
    O -.->|Vnitřní konflikt a stres| K["Rozpor v identitě"]
    
    style O fill:#2c3e50,stroke:#fff,color:#fff
    style K fill:#8b0000,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    A["Biologická a Ekonomická funkce<br>(Jídlo, teplo, bydlení)"] --> B["Emocionální funkce<br>(Pocit bezpečí, láska)"]
    B --> C["Výchovná a Socializační funkce<br>(Učení hodnot a rolí)"]
    
    style A fill:#2c3e50,stroke:#fff,color:#fff
    style B fill:#e67e22,stroke:#fff,color:#fff
    style C fill:#006400,stroke:#fff,color:#fff
```
...
```mermaid
graph LR
    R["Dysfunkční rodiče<br>(Křik a ignorace)"] -->|"Dítě to vidí jako normu"| D(("Dítě v rodině"))
    D -->|"Přenos vzorce do školy"| S["Zlobivé / agresivní chování<br>ve školní třídě"]
    S -->|"Reakce neznalého učitele"| U["Tresty, pětky, ponížení před třídou"]
    U -.->|"Prohloubení deprivace"| D
    
    style R fill:#8b0000,stroke:#fff,color:#fff
...
```mermaid
graph LR
    A["Křečovité pohyby<br>(Zbytečná síla, chyby)"] -->|Opakování a Dril| B["Eliminace chyb<br>(Zrychlení, upřesnění)"]
    B -->|Zafixování do podvědomí| C["AUTOMATICKÝ NÁVYK<br>(Pohyb je ladný, nezatěžuje vědomí)"]
...
```mermaid
graph TD
    subgraph TRADICNI ["Tradiční škola (Učení poznatkům)"]
        T1["Učitel přednáší fakta"] --> T2["Žák paměťově ukládá"] --> T3["Výsledek: Teoretická Vědomost"]
    end
    
    subgraph MODERNI ["Moderní škola (Řešení problémů)"]
        M1["Učitel zadá problémový úkol"] --> M2["Žák analyzuje, tvoří, hledá řešení"] --> M3["Výsledek: Praktická Kompetence"]
    end
    
    style TRADICNI fill:#1a1a2e,stroke:#fff,color:#fff
    style MODERNI fill:#006400,stroke:#fff,color:#fff
```
...
```mermaid
graph LR
    A["Agresivní (Autoritativní)"] -->|"Dělej, co ti říkám!"| B(("Pocity žáka:<br>Strach, Útěk"))
    C["Pasivní (Lhostejný)"] -->|"To je tvůj problém."| D(("Pocity žáka:<br>Opuštění, Rezignace"))
    E["PORADENSKÝ (Empatický)"] -->|"Rozumím, co tě trápí.<br>Jaké vidíš možnosti?"| F(("Pocity žáka:<br>Důvěra, Růst"))
...
```mermaid
graph TD
    S1["1. Ostrakismus<br>(Izolace odlišného žáka, lehké vtípky)"] --> S2["2. Přitvrzování<br>(Zkoušení hranic - první šťouchance, urážky)"]
    S2 --> S3["3. Vytvoření Jádra<br>(Agresor získává partu následovníků)"]
    S3 --> S4["4. Přijetí norem většinou<br>(Třída se přidává ze strachu / fascinace)"]
    S4 --> S5["5. TOTALITA ŠIKANY<br>(Dokonalé ovládnutí třídy agresorem, zlomení oběti)"]
...
```mermaid
graph TD
    A["Kojenec<br>(Důvěra vs. Nedůvěra)"] --> B["Batole<br>(Autonomie vs. Stud)"]
    B --> C["Předškolák<br>(Iniciativa vs. Vina)"]
    C --> D["Mladší Školák<br>(Snaživost vs. Méněcennost)"]
    D --> E["Adolescent - Středoškolák<br>(Hledání identity vs. Zmatení)"]
    E --> F["Mladý dospělý<br>(Intimita vs. Izolace)"]
    F --> G["Střední věk<br>(Produktivita vs. Stagnace)"]
    G --> H["Stáří<br>(Integrita ega vs. Zoufalství)"]
...
```mermaid
graph LR
    P1["Senzomotorické<br>(0-2 r.)<br>Zkoumá svět pusou"] --> P2["Předoperační<br>(2-7 r.)<br>Magické myšlení, slova"]
    P2 --> P3["Konkrétní operace<br>(7-11 r.)<br>Logika s kostkami"]
    P3 --> P4["FORMÁLNÍ OPERACE<br>(12+ let)<br>Abstrakce, filozofie, hypotézy"]
