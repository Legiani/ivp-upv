...
```mermaid
graph TD
    subgraph TROJUHELNIK ["Jádro výuky"]
        U["👩‍🏫 UČITEL<br>(Expert z praxe s pedagogickým taktem)"] 
        Z["🧑‍🎓 ŽÁK<br>(Budoucí profesionál)"]
        O["📚 UČIVO<br>(Odborný obsah zjednodušený pro výuku)"]
        
        U <==>|"Motivuje a vede"| Z
        U <==>|"Didakticky transformuje"| O
        Z <==>|"Osvojuje si a zkoumá"| O
    end
    
    P["🏭 PROSTŘEDÍ (Dílna, trh práce, laboratoř)"]
    P -.->|"Definuje nároky a podmínky"| TROJUHELNIK
    
    style U fill:#0f3460,stroke:#fff,color:#fff
    style Z fill:#16213e,stroke:#fff,color:#fff
    style O fill:#533483,stroke:#fff,color:#fff
    style P fill:#8b0000,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    STAT["🏢 STÁTNÍ ÚROVEŇ<br>NÚV / MŠMT"] -->|Vydává| RVP["📖 RVP (Rámcový vzděl. program)<br>Co má žák na konci umět (Kompetence)"]
    
    RVP -->|Je závazný pro| SKOLA["🏫 ŠKOLNÍ ÚROVEŇ<br>Ředitel a učitelé"]
    
    SKOLA -->|Vytváří na míru| SVP["📘 ŠVP (Školní vzděl. program)<br>Jak a v kterém ročníku se k cíli dostaneme"]
    
    SVP --> OSNOVY["📋 Učební osnovy konkrétního předmětu"]
    OSNOVY --> UCITEL["👨‍🏫 Plán práce učitele (Tematický plán)"]
    
    style STAT fill:#2c3e50,stroke:#fff,color:#fff
    style SKOLA fill:#2c3e50,stroke:#fff,color:#fff
    style RVP fill:#e67e22,stroke:#fff,color:#fff
    style SVP fill:#27ae60,stroke:#fff,color:#fff
    style OSNOVY fill:#2980b9,stroke:#fff,color:#fff
    style UCITEL fill:#8e44ad,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    subgraph BEZ_FIXACE ["Výuka bez opakování"]
        A1["100 % (Konec výkladu)"] -->|Za 20 minut| B1["58 %"]
        B1 -->|Za 1 den| C1["33 %"]
        C1 -->|Za 1 měsíc| D1["21 % (Trvalá paměť)"]
    end
    
    subgraph S_FIXACI ["S pravidelnou fixací"]
        A2["100 %"] -->|Rychlé opáčko| B2["90 %"]
        B2 -->|Opáčko další den| C2["85 %"]
        C2 -->|Test po týdnu| D2["80 % (Trvalá paměť)"]
    end
    
    style BEZ_FIXACE fill:#8b0000,stroke:#fff,color:#fff
    style S_FIXACI fill:#006400,stroke:#fff,color:#fff
```
...
```mermaid
gantt
    title Běžná kombinovaná hodina
    dateFormat YYYY-MM-DD HH:mm
    axisFormat %H:%M
    
    Organizace - Zapis a dochazka : 2024-01-01 08:00, 3m
    Diagnostika - Zkouseni : 2024-01-01 08:03, 10m
    Expozice - Vyklad noveho uciva : 2024-01-01 08:13, 20m
    Fixace - Shrnuti a priklady : 2024-01-01 08:33, 10m
    Zaver - Uklid a hodnoceni : 2024-01-01 08:43, 2m
```
...
```mermaid
graph TD
    TEMA(("Vstupní téma<br>(např. Svařování)"))
    
    A1["🔍 1. Pojmová analýza<br>(Sepsání všech odborných termínů)"]
    A2["✂️ 2. Výběr základního učiva<br>(Redukce informací)"]
    A3["🔗 3. Mezipředmětové vztahy<br>(Co znají z fyziky a chemie?)"]
    A4["🎯 4. Stanovení cílů<br>(Co bude žák umět?)"]
    A5["🛠️ 5. Volba metod a forem<br>(Jak to naučím?)"]
    
    TEMA --> A1 --> A2 --> A3 --> A4 --> A5
    
    style TEMA fill:#e94560,color:#fff
...
```mermaid
graph LR
    P["Předhodinový pohovor<br>(Cíle hodiny)"] --> H["Samotná hospitace<br>(Pozorování, záznam)"]
    H --> R["Pohospitační rozbor<br>(Sebereflexe učitele + Zpětná vazba)"]
    R --> Z["Hospitační záznam<br>(Doporučení a podpis)"]
    
    style P fill:#1a1a2e,stroke:#fff,color:#fff
    style H fill:#e94560,stroke:#fff,color:#fff
    style R fill:#006400,stroke:#fff,color:#fff
    style Z fill:#533483,stroke:#fff,color:#fff
```
...
```mermaid
graph TD
    A["🏆 Pracovní postup<br>(Komplexní výrobek / služba)"] --> B1["⚙️ Pracovní operace 1<br>(Např. příprava materiálu)"]
    A --> B2["⚙️ Pracovní operace 2<br>(Např. obrábění)"]
    A --> B3["⚙️ Pracovní operace 3<br>(Např. montáž)"]
    
    B2 --> C1["🤏 Úkon 1 (Upnutí)"]
    B2 --> C2["🤏 Úkon 2 (Nastavení)"]
    B2 --> C3["🤏 Úkon 3 (Spuštění)"]
...
```mermaid
graph LR
    F1["1. Motivace"] --> F2["2. Instruktáž<br>(Ukázka mistra)"]
    F2 --> F3["3. Imitace<br>(První pokus žáka)"]
    F3 --> F4["4. Dril<br>(Vzniká návyk)"]
    F4 --> F5["5. Aplikace<br>(Použití v praxi)"]
...
```mermaid
graph TD
    A["🎓 Odborný výcvik a Praxe"]
    
    A --> B1["🏫 Formy ve škole"]
    B1 --> C1["Školní dílny (1. a 2. ročník)"]
    B1 --> C2["Cvičné firmy / laboratoře"]
    
    A --> B2["🏭 Formy v reálných podnicích"]
    B2 --> D1["Exkurze (Pasivní pozorování)"]
    B2 --> D2["Skupinová praxe s mistrem"]
    B2 --> D3["Individuální provozní praxe s instruktorem<br>(3. a 4. ročník - Nejvyšší stupeň)"]
    
    style A fill:#533483,stroke:#fff,color:#fff
    style B1 fill:#2c3e50,stroke:#fff,color:#fff
    style B2 fill:#8b0000,stroke:#fff,color:#fff
...
```mermaid
gantt
    title Příklad - 7 hodinový učební den od 7:00 do 13:30
    dateFormat YYYY-MM-DD HH:mm
    axisFormat %H:%M
    
    section Uvod
    Nastup, BOZP a Instruktaz : 2024-01-01 07:00, 45m
    
    section Pracovni cast
    Samostatna prace zaku : 2024-01-01 07:45, 120m
    Prestávka a Svacina : 2024-01-01 09:45, 30m
    Samostatna prace - Dokoncovani : 2024-01-01 10:15, 135m
    
    section Zaver
    Vyhodnoceni praci a Reflexe : 2024-01-01 12:30, 30m
    Uklid a kontrola naradi : 2024-01-01 13:00, 30m
```
...
```mermaid
pie title Z čeho se skládá známka v odborném výcviku?
    "Přesnost a funkčnost (Výkres)" : 45
    "Dodržení postupu a Samostatnost" : 20
    "BOZP a ochranné pomůcky" : 15
    "Pracovní tempo" : 10
    "Úklid pracoviště a nástrojů" : 10
```
...
```mermaid
graph TD
    A["T-1 Den<br>Předchozí odpoledne"] --> B["Kontrola strojů a nařezání materiálu"]
    A --> C["Příprava dokumentace (Výkresy)"]
    
    D["T-0<br>Ráno (06:30)"] --> E["Zapnutí hlavních rozvaděčů, otevření dílen"]
    D --> F["Zápis do Deníku OV (Příprava prezence a BOZP podpisů)"]
    
    G["T+0<br>Příchod žáků (07:00)"] --> H["Nástup, vizuální kontrola žáků (alkohol/únava/oděv)"]
    
    style A fill:#2c3e50,stroke:#fff,color:#fff
    style D fill:#e67e22,stroke:#fff,color:#fff
    style G fill:#8b0000,stroke:#fff,color:#fff
```
